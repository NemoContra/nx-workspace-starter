...
const flightServiceMock = {
  ...
  load(from: string, to: string): void {
    this.find(from, to).subscribe((f: Flight[]) => {
      this.flights = f;
    });
  },
};
...
describe('flight-search.component', () => {
  ...
  it('should have a disabled search button w/o params', () => {
    ...
    const from = fixture.debugElement.query(
      By.css('input[name=from]')
    ).nativeElement;
    ...
    const to = fixture.debugElement.query(
      By.css('input[name=to]')
    ).nativeElement;
    ...
    // get disabled
    const disabled = fixture.debugElement.query(By.css('button')).properties[
      'disabled'
    ];
