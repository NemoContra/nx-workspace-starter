...
export class FlightResolver {
  ...
  async searchFlights(
    @Args({ type: () => FlightSearchArgs })
    { from, to, fromDate, toDate }: FlightSearchArgs
  ): Promise<Flight[]> {
  ...
  async createFlight(
    @Args({ name: 'flight', type: () => CreateFlight }) flight: CreateFlight
  ): Promise<Flight> {
  ...
  @Mutation(() => FlightObjectType)
  async updateFlight(
    @Args({ name: 'flight', type: () => UpdateFlight }) flight: UpdateFlight
  ): Promise<Flight> {
